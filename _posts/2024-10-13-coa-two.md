...
There are some figures to remember. 
    - Address length = (s+w) bits, where the **most significant s bits** specify a memory block, while the **least significant w bits** specify a word within the block.
