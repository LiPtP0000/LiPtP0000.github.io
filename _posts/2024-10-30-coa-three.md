...
{:.box-note}
For *unconditional branch*, the branch will be always taken, and the execution will be ended at `CO` stage. For *conditional branch*, the branch will be taken only if the condition is satisfied. For *skips*, the **next instruction** will be skipped if condition is satisfied.

{:.box-note}
For a **procedure**, it can be called from more than one location, *i.e.* **reentrant procedure**, and allows **nesting**(嵌套). However each procedure `call` requires a `return`. The return address are stored at **Register**, **Top of a stack**, or the **start place of called procedure**. 
